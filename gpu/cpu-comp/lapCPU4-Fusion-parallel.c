...
int iter;
float error;
#pragma omp threadprivate(error, iter)
inline float stencil ( float v1, float v2, float v3, float v4)
{
	return (v1 + v2 + v3 + v4) * 0.25f;
}

inline float max_error ( float prev_error, float old, float new )
{
...
int main(int argc, char** argv)
{
	...
	int iter = 0;
	#pragma parallel firstprivate(A, temp) copyin(iter, error)
	{
