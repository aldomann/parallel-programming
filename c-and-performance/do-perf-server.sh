...
echo -n $(perf stat -o perf-out.txt -e cycles,instructions ./diffrun $NX $NY $NZ | cut -d ' ' -f8) >> perf-res-diff.csv
...
echo -n "," >> perf-res-diff.csv
echo -n $(grep "time elapsed" perf-out.txt | awk '{print substr($1,0,19) }' | sed 's/,/\./g') >> perf-res-diff.csv
